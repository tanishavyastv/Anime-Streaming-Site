...
function loadMoreAiring(event) {
  ...
  const loadMoreButton = event?.target || document.querySelector('button[onclick="loadMoreAiring(event)"]');
      ...
      <div class="col-md-6 col-xl-4">
        <div class="anime-card p-3 h-100 w-100">
          ...
          <div class="mt-auto d-flex justify-content-between">
            ...
            <button class="btn btn-sm btn-outline-warning" data-bs-toggle="modal" data-bs-target="#trailerModal${index}">Watch Trailer</button>
            <button class="btn btn-sm btn-pink" data-bs-toggle="modal" data-bs-target="#infoModal${index}">More Info</button>
          </div>
        </div>
      </div>
  ...
  aired += perLoad;
  if (loadMoreButton && aired >= airingAnime.length) {
    loadMoreButton.style.display = "none";
  }
}

window.addEventListener("load", () => loadMoreAiring());
